...
class NovalnetCashPayment implements SynchronousPaymentHandlerInterface
{
    /**
    ...
    {
        $responseParams = $this->helper->getRequestParams($salesChannelContext, $transaction);
        $this->addNovalnetTransactionTid($transaction, $responseParams, $salesChannelContext->getContext());

        if (!empty($responseParams['tid_status']) && in_array($responseParams['tid_status'], [100, 91])) {
